...
class AtraksiService:
    ...
    @rpc
    def get_all_atraksi(self, id_lokasi, attractioname, tanggal, minprice, maxprice,  sort):
        ...
        for atraksi_service in atraksi_services:
            ...
            # url: http://3.217.250.166:8003/api/atraksi -> mendapatkan info dari atraksi
            # url: http://3.217.250.166:8003/api/atraksi/paket/<int:id_paket> -> mendapatkan info paket berdasarkan id paket
            # url: http://3.217.250.166:8003/api/atraksi/paket -> mendapatkan semua list paket
            # url: http://3.217.250.166:8003/api/eticket -> untuk meminta pembutan eticket
            # url: http://3.217.250.166:8003/api/atraksi/tutup/<string:tgls> -> untuk mengecek atraksi tutup atau tidak
            # url: http://3.217.250.166:8003/api/eticket/<string:booking_code> -> untuk mendapatkan ticket yang sudah ada
            ...
            try:
                ...
                if sort == 'lowestprice':
                    atraksis = sorted(atraksis, key=lambda x: x['price'])
                elif sort == 'highestprice':
                    atraksis = sorted(atraksis, key=lambda x: x['price'], reverse=True)
                elif sort == 'highestpopularity':
                    atraksis = sorted(atraksis, key=lambda x: x['popularity'], reverse=True)
